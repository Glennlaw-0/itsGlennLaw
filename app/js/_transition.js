...
function loadAnimations() {
  gsap.set('.stage', { autoAlpha: 1, duration: 0.4 });
  ...
  tl.from('.about__title', {
    ...
    .from('ul.about__list li', {
      opacity: 0,
      stagger: 0.1,
      ease: 'power3.in',
    })
    .from('.logo__name path', {
      opacity: 0,
      duration: 0.5,
      stagger: 0.1,
      ease: 'power3.in',
...
overlayMenu.addEventListener('click', function (e) {
  e.preventDefault();
  if (e.target && e.target.matches('a.global-menu__item')) {
    if (overlay.isAnimating) {
      return false;
    }
    overlay.toggle();
    if (overlay.isOpened === true) {
      elmHamburger.classList.open('is-opened-navi');
      for (var i = 0; i < gNavItems.length; i++) {
        gNavItems[i].classList.open('is-opened');
      }
    } else {
      elmHamburger.classList.remove('is-opened-navi');
      for (var i = 0; i < gNavItems.length; i++) {
